...
namespace ntt {
  ...
  auto Particles<D, C>::NpartsPerTagAndOffsets() const
    ...
    //
    // The previous implementation used a scatter_view, which on Aurora's
    // SYCL backend cost ~960 ms / call (96% of Comm.particles): one bin
    // (alive) holds 95-99% of particles, and either the per-team
    // duplicated buffers it allocated each call or the contention on the
    // alive bin dominated. Reading npptag(alive) directly is unnecessary
    // because npart() is the total — alive is reconstructed on host as
    // (npart() - sum_of_other_bins).
  ...
  void Particles<D, C>::SortSpatially(const Grid<D>& grid) {
    ...
    // 2. Compute per-particle tile key (with min(i, i_prev)). NO atomics
    //    — counts come out of BinSort below, not from a custom histogram.
    //    The atomic_add to a custom `num_ppt` View was the source of a
    //    GPU "NotPresent atomic" page fault on PVC (Aurora oneAPI 25.18);
    //    using BinSort's internal counts mirrors the legacy SortSpatially
    //    memory and atomic profile and avoids the issue.
    ...
    // matching legacy memory profile).
    //
    // `sort_by_key` (oneDPL/Thrust) sorts `tile_indices` in place
    // alongside the produced `perm`, so post-call `tile_indices` is
    // monotonically non-decreasing. Carry it through to the offsets
    // pass (§5) — re-allocating a 720 MB recompute buffer was the
    // single biggest source of the +800 ms sort regression.
    ...
    // monotonically non-decreasing for the offsets pass below. Cheaper
    // than allocating a 720 MB recompute buffer (which Direction A
    // identified as the source of the +800 ms sort regression).
    sorter.sort(tile_indices);
  #endif // TEAM_POLICY_USE_VENDOR_SORT

    // 5. Compute per-tile prefix-sum `tile_offsets` for the tiled
    //    pusher (Stream 3). `tile_indices` is now sorted (monotonically
